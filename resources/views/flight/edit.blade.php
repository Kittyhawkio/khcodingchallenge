...
@section('body')
<form id="edit-form" method="POST" action="/flights/{{ $flight->id }}">
    @method('PATCH')
    @csrf
    <div class="flex-col flex justify-between">

    {{-- Latitude & Longitude --}}
    <div class="flex-col md:flex-row flex justify-start my-5">
        <div class="w-full md:w-1/2 mr-5">
            {{-- Latitude --}}
            <label class="font-bold" for="latitude">Latitude:</label>
            <input class="w-1/2 text-right bg-gray-100 border border-gray-200 rounded @error('latitude') border-red-600 @enderror"
                step=0.05 type="number" id="latitude" name="latitude"
                min="-90" max="90" value={{ old('latitude') ?? $flight->lat }} />°N
        </div>
        <div class="w-full md:w-1/2 mr-5">
            {{-- Longitude --}}
            <label class="font-bold" for="longitude">Longitude:</label>
            <input class="w-1/2 text-right bg-gray-100 border border-gray-200 rounded @error('longitude') border-red-600 @enderror"
                step=0.05 type="number" id="longitude" name="longitude"
                min="-180" max="180" value={{ old('longitude') ?? $flight->long }} />°E
        </div>
    </div>
    ...
    <div class="my-5">
        ...
        <input class="text-right bg-gray-100 border border-gray-200 rounded @error('date') border-red-600 @enderror"
            type="datetime-local" id="date" name="date" value="{{ old('date') ?? $flight->flight_time->isoFormat('YYYY-MM-DDThh:mm') }}">
    </div>
    ...
    <div class="my-5">
        ...
        <input class="text-right bg-gray-100 border border-gray-200 rounded @error('duration_in_seconds') border-red-600 @enderror"
            step=1 type="number" id="duration_in_seconds" name="duration_in_seconds" min="0"
            value={{ old('duration_in_seconds') ?? $flight->duration_in_seconds }} /> seconds
    </div>
    ...
    <div class="my-10">
        ...
        <textarea id="notes" name="notes"
            ...
            >{{ old('notes') ?? $flight->notes }}</textarea>
    </div>
...
<script>
function back() {
    if (! document.referrer.endsWith('edit')) {
        window.history.back();
    } else {
